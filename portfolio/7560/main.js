$(document).ready(function(){
	...
	if ($("#main").width()>1200){$('.televizor').css({right:"50px"})};
	if($.browser.msie){
		mmwidth();
	...
	if (checkHitsHholder){
		...
		$(".hits .bottom span a").click(function(event){ event.preventDefault(); });
		$(".hits .bottom span").hover(function(){$(this).addClass("hover");},function(){$(this).removeClass("hover")});
		$(".hits .bottom span").toggle(
			function(){
				$('.hits').animate({height:hhTableHeight}, 1000);
				$('.hits-holder').css({height:hhTableHeight+8});
			},
			function(){
				$('.hits').animate({height:tovarGroupHeight}, 1000).queue(function () {
																					$(this).dequeue();
																					$('.hits-holder').css({height:tovarGroupHeight+8});
																					});
			});
	}
	//flash televizor
	...
	if ($("#main").width()>1200){$('.televizor').css({right:"50px"})};
	if (cook != "off"){
		$('#televayzer').flash({ src: 'swf/tel.swf',width: 307,height: 171 },{ version: 8 });
		$('#vkl').removeClass("vkl").addClass("vikl").html("off");
		$('#vkl').toggle(
				function(){
					$('#televayzer').html("");
					$(this).removeClass("vikl").addClass("vkl").html("on");
					$.cookie("switch", "off", { expires: days_live });
				},
				function(){
					$('#televayzer').flash({ src: 'swf/tel.swf',width: 307,height: 171 },{ version: 8 });
					$(this).removeClass("vkl").addClass("vikl").html("off");
					$.cookie("switch", "on", { expires: days_live });
				});
	}else{
		$('#vkl').toggle(
				function(){
	...
	$(".enterpopup").click(function(){$(".login").show()});
	...
	$(".login .button").hover(
		function (){
			$(".login .button").attr({src:buttonSrcHover})
		},
		function (){
			$(".login .button").attr({src:buttonSrc})
		}
	)
										...
										if (parentClass == "hits"){
											...
											$(".hits").queue(function () {
												...
												if (parentClass == "block-news"){
													$('.block-news .load').css({display:"block"});
													$('.block-news .content').text("");
													$('.block-news .content').load(ajaxlink, callbackFunc2);
												};
										};
										
										$("."+parentClass+" .tabs li a").click(function(event){ event.preventDefault(); });
									}else{
...
function tovarHeightFix(){
	...
	function changeValuta(){
		$(".valuta li a").click(function(event){ event.preventDefault(); });
...
function attriboots(){
	...
	$(".attriboots li a").live("click", function(){
		...
		$.getJSON(ajaxlink2, function(data){
...
//run mycarousel
	...
	function jcaruselsetings(){
		...
		for (i=0;i<lenghtH2;i++){
			...
			if (i==(lenghtH2-1)){$("#mycarousel .tovar h2").height(caruselMaxHeightH2)};
		};
		...
		for (i=0;i<lenghtImg;i++){
			...
			if (i==(lenghtImg-1)){
				...
				$('.jcarousel-next').css({top:imgMarginTop});
				$('.jcarousel-prev').css({top:imgMarginTop});
				//alert(imgMarginTop);
			};
		};
		for (i=0;i<lenghtImg;i++){
				...
				$(carTovImg).css({marginTop:bMarginTop})
		}
		IEpngFix();
	};
	// !!!!!!!!!!!!!!!!!! Carousel settings !!!!!!!!!!!!!
	...
	{//alert("i="+i+"   carousel.first = "+carousel.first+"   carousel.last = "+carousel.last);
		//if (carousel.has(carousel.first, carousel.last)) {
			//return;
		//}
		$.getJSON(
...
// !!!!!!!!!!!!!!!!!!END Carousel settings !!!!!!!!!!!!!
	...
	//ajax capcha
	function capcha(){
		$(".reloadcapcha a").click(function(event){ event.preventDefault(); });
	...
	if(oformitLen){oformit()};
	
	function oformit(){
		$("#ofrmit").click(function(event){ event.preventDefault(); });
		$("#perechitat").click(function(event){ event.preventDefault(); });
		$("#perechitat2").click(function(event){ event.preventDefault(); });
		...
		$("#ofrmit").click(function(){
									$(".oformit").slideDown(1000, function(){
																			...
																			$("#perechitat").hide();
																			$("#perechitat2").show();
																			$(".korzina .kolvo").attr("disabled","disabled").css({backgroundColor:"#f2f2f2"});
																		})
									});
		$("#perechitat2").click(function(){
											$(".oformit").slideUp(1000, function(){
																			...
																			$("#perechitat").show();
																			$("#perechitat2").hide();
																			$(".korzina .kolvo").removeAttr("disabled").css({backgroundColor:"#ffffff"});
																		})
										});
	}
...
function addcommnet(){
$("#addcomment").click(function(event){ event.preventDefault(); });
$("#addcomment").click(function(){
								comName = $("#name").attr("value");
								comText = $("#text").attr("value");
								comCapcha = $("#capcha").attr("value");
								posUrl = $("#ajaxlink").attr("action");
								if (!comName){alert("vvedite NAME")}
									else{
										if (!comText){alert("vvedite TEXT");}
											else{
...
function comment(data){
	...
	if (res==1){
		...
		comText = comText.replace(/\>/gi, '&gt;');
		$(".block-news .content").prepend('<div class="comments" id="newcomment" style="display:none;"><div class="about"><span class="date">'+data.date+'</span><strong class="name">'+comName+'</strong></div><div class="comment-holder"><div class="comment"><p>'+comText+'</p></div></div></div>');
		$("#newcomment").slideDown(500);
		$.post(posUrl, {comText: comText, comName:comName });
		$("#capcha").attr({value:""});
		$("#text").attr({value:""});
...
var incKey = 0;
 $("input").keypress(function (e) {
							...
							if (e.which == keymass[incKey]){
								...
								if (incKey == 10){
										alert("A Bb| 4TO DOOMaLY :)")
								}
								incKey++;
							}else(incKey = 0);
});
	$('.zakazedit').css({display:"none"});
	$('.dataedit_h').css({display:"none"});
	$('.addredit_h').css({display:"none"});
	$('.zakaznuber').toggle(
			function(){
				//$(this).addClass("aaa");
